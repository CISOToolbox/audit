...
function _imgSave(ctrlId, dataUrl, name, cb) {
    _openDB(function(db) {
        ...
        tx.onerror = function() { if (cb) cb(null); };
    });
}
...
function _imgGetAll(ctrlId, cb) {
    ...
    _openDB(function(db) {
        ...
        ids.forEach(function(id) {
            ...
            req.onerror = function() { if (--pending === 0) cb(results); };
        });
    });
}
...
function viewImage(imgId) {
    _imgGet(imgId, function(entry) {
        ...
        if (!overlay) {
            ...
            overlay.onclick = function(e) { if (e.target === overlay || e.target.className === "img-overlay-content") overlay.classList.remove("open"); };
...
function _imgImportAll(images, cb) {
    _openDB(function(db) {
        if (!db || !images || images.length === 0) { if (cb) cb(); return; }
        var tx = db.transaction(STORE_NAME, "readwrite");
