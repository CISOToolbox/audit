...
function selectPanel(panelId) {
    ...
    if (panelId === "dashboard")
        ...
        renderJournal();
    else if (panelId === "history" && typeof renderHistory === "function")
        renderHistory();
}
...
function renderDomain(domainId) {
    ...
    controls.forEach(function (c) {
        ...
        if (_filterText) {
            ...
            if (c.id.toLowerCase().indexOf(q) < 0 && c.t.toLowerCase().indexOf(q) < 0 && c.desc.toLowerCase().indexOf(q) < 0 && (f.constats || "").toLowerCase().indexOf(q) < 0 && (f.preuve || "").toLowerCase().indexOf(q) < 0)
                return;
        }
        h += cardHTML(c, f);
        shown++;
...
function buildDonut(S) {
    ...
    if (todo > 0)
        data.push({ label: t("audit.dash.non_audited"), value: todo, color: "#cbd5e1" });
    if (data.length === 0)
        return '<div style="text-align:center;color:var(--text-muted);padding:20px">&mdash;</div>';
...
function buildDomainBars(S) {
    ...
    DOMAINS.forEach(function (dom) {
        ...
        if (ds.c > 0)
            h += '<div style="position:absolute;height:100%;width:' + (ds.c / ds.total * 100) + '%;background:#16a34a"></div>';
        if (ds.ncmaj > 0)
            h += '<div style="position:absolute;height:100%;left:' + (ds.c / ds.total * 100) + '%;width:' + (ds.ncmaj / ds.total * 100) + '%;background:#dc2626"></div>';
        if (ds.ncmin > 0)
            h += '<div style="position:absolute;height:100%;left:' + ((ds.c + ds.ncmaj) / ds.total * 100) + '%;width:' + (ds.ncmin / ds.total * 100) + '%;background:#f59e0b"></div>';
        if (ds.ps > 0)
            h += '<div style="position:absolute;height:100%;left:' + ((ds.c + ds.ncmaj + ds.ncmin) / ds.total * 100) + '%;width:' + (ds.ps / ds.total * 100) + '%;background:#7c3aed"></div>';
        if (ds.pp > 0)
            h += '<div style="position:absolute;height:100%;left:' + ((ds.c + ds.ncmaj + ds.ncmin + ds.ps) / ds.total * 100) + '%;width:' + (ds.pp / ds.total * 100) + '%;background:#3b82f6"></div>';
        h += '</div>';
...
function renderJournal() {
    ...
    D.journal.forEach(function (entry) {
        ...
        if (entry.author)
            h += '<span style="font-size:0.8em;color:var(--text-muted)">' + esc(entry.author) + '</span>';
        h += '</div>';
    });
    el.innerHTML = h;
}
...
function renderAll() {
    try {
        ...
        if (_currentPanel === "dashboard")
            ...
            renderPlanning();
        else if (_currentPanel === "history" && typeof renderHistory === "function")
            renderHistory();
    }
    catch (e) {
