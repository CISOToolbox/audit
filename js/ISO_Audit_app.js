...
function onFilterStatus(val) { _filterStatus = val; if (_currentPanel.startsWith("domain-")) renderDomain(_currentPanel.replace("domain-", "")); }
function onFilterHDS(val) { _filterHDS = val; if (_currentPanel.startsWith("domain-")) renderDomain(_currentPanel.replace("domain-", "")); }
function onFilterText(val) { _filterText = val; if (_currentPanel.startsWith("domain-")) renderDomain(_currentPanel.replace("domain-", "")); }
...
function computeStats() {
    var S = { total: CONTROLS.length, audited: 0, c: 0, ncmaj: 0, ncmin: 0, ps: 0, pp: 0, na: 0 };
    CONTROLS.forEach(function(ctrl) {
        var f = getFinding(ctrl.id);
        if (f.status) {
            S.audited++;
            if (S[f.status] !== undefined) S[f.status]++;
        }
    });
    ...
    DOMAINS.forEach(function(dom) {
        ...
        var ds = { total: ctrls.length, audited: 0, c: 0, ncmaj: 0, ncmin: 0, ps: 0, pp: 0, na: 0 };
        ctrls.forEach(function(ctrl) {
            var f = getFinding(ctrl.id);
            if (f.status) {
                ds.audited++;
                if (ds[f.status] !== undefined) ds[f.status]++;
            }
        });
...
function _exportAIReportAsWord(text) {
    _loadAsset("https://cdn.jsdelivr.net/npm/jszip@3.10.1/dist/jszip.min.js", function() {
        ...
        var fname = (m.ref || "audit").replace(/[^a-zA-Z0-9\-_]/g, "_") + "_rapport_IA.docx";
...
function ensureKeys() {
    ...
    for (var k in metaDefaults) { if (!(k in D.meta)) D.meta[k] = metaDefaults[k]; }
...
async function renderHistory() {
    ...
    h += '<button class="btn-add" data-click="createSnapshot">' + t("audit.history.create") + '</button>';
    if (isEnc) {
        h += '<button class="btn-add" style="background:var(--red)" data-click="disableSnapEncryption">' + t("audit.history.decrypt") + '</button>';
        h += '<span style="color:var(--green);font-size:0.82em">' + t("audit.history.encryption_active") + '</span>';
    } else {
        h += '<button class="btn-add" style="background:var(--light-blue)" data-click="enableSnapEncryption">' + t("audit.history.encrypt") + '</button>';
    }
    h += '</div>';
    if (snaps.length === 0) {
        h += '<p style="color:var(--text-muted)">' + t("audit.history.none") + '</p>';
    } else {
        h += '<table class="nc-table"><thead><tr><th>' + t("audit.history.col_name") + '</th><th>' + t("audit.history.col_date") + '</th><th>' + t("audit.history.col_client") + '</th><th>' + t("audit.history.col_actions") + '</th></tr></thead><tbody>';
        for (var i = snaps.length - 1; i >= 0; i--) {
            ...
            h += '<tr><td><strong>' + esc(s.name) + '</strong></td><td>' + dateStr + '</td><td style="font-size:0.85em">' + esc(s.societe || "") + '</td>';
            h += '<td><button class="btn-add" style="margin-right:4px" data-click="restoreSnapshot" data-args=\'' + _da(i) + '\'>' + t("audit.history.restore") + '</button>';
            h += '<button class="btn-add" style="margin-right:4px;background:var(--light-blue)" data-click="exportSnapshot" data-args=\'' + _da(i) + '\'>' + t("audit.history.export") + '</button>';
            h += '<button class="btn-del" data-click="deleteSnapshot" data-args=\'' + _da(i) + '\'>X</button></td></tr>';
        }
        h += '</tbody></table>';
    }
    h += '<p style="margin-top:16px;color:var(--text-muted);font-size:0.82em">' + t("audit.history.hint") + '</p>';
    el.innerHTML = h;
}
