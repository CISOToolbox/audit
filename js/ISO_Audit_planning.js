...
// ISO Audit — PLANNING
// ═══════════════════════════════════════════════════════════════════════
var PLANNING_DOMAINS = window.ISO_AUDIT_DOMAINS || [];
// ── HELPERS ──
...
function _domainGroup(domainId) {
    for (var i = 0; i < PLANNING_DOMAINS.length; i++) {
        if (PLANNING_DOMAINS[i].id === domainId)
            return PLANNING_DOMAINS[i].group;
    }
    return "";
}
function _domainLabel(domainId) {
    for (var i = 0; i < PLANNING_DOMAINS.length; i++) {
        if (PLANNING_DOMAINS[i].id === domainId)
            return PLANNING_DOMAINS[i].label;
    }
    return domainId;
}
// ── RENDER ──
function renderPlanning() {
    ...
    var h = '';
    // ── Parameters section ──
    h += '<div class="planning-params" style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px;margin-bottom:20px;padding:16px;background:var(--bg);border-radius:8px;border:1px solid var(--border)">';
    h += '<div class="planning-param" style="display:flex;flex-direction:column;gap:4px">';
    h += '<label style="font-size:0.75em;font-weight:600;color:var(--text-muted);text-transform:uppercase">' + t("audit.planning.start_date") + '</label>';
    h += '<input type="date" style="padding:6px 8px;border:1px solid var(--border);border-radius:4px;font-size:0.85em" value="' + esc(p.start_date || "") + '" data-change="onPlanningParam" data-args=\'' + _da("start_date") + '\' data-pass-value>';
    h += '</div>';
    h += '<div class="planning-param" style="display:flex;flex-direction:column;gap:4px">';
    h += '<label style="font-size:0.75em;font-weight:600;color:var(--text-muted);text-transform:uppercase">' + t("audit.planning.days") + '</label>';
    h += '<input type="number" min="1" max="10" style="padding:6px 8px;border:1px solid var(--border);border-radius:4px;font-size:0.85em" value="' + esc(String(p.days || 3)) + '" data-change="onPlanningParam" data-args=\'' + _da("days") + '\' data-pass-value>';
    h += '</div>';
    h += '<div class="planning-param" style="display:flex;flex-direction:column;gap:4px">';
    h += '<label style="font-size:0.75em;font-weight:600;color:var(--text-muted);text-transform:uppercase">' + t("audit.planning.start_time") + '</label>';
    h += '<input type="time" style="padding:6px 8px;border:1px solid var(--border);border-radius:4px;font-size:0.85em" value="' + esc(p.start_time || "09:00") + '" data-change="onPlanningParam" data-args=\'' + _da("start_time") + '\' data-pass-value>';
    h += '</div>';
    h += '<div class="planning-param" style="display:flex;flex-direction:column;gap:4px">';
    h += '<label style="font-size:0.75em;font-weight:600;color:var(--text-muted);text-transform:uppercase">' + t("audit.planning.slot_duration") + '</label>';
    h += '<div style="display:flex;align-items:center;gap:6px">';
    h += '<input type="number" min="15" max="240" step="15" style="padding:6px 8px;border:1px solid var(--border);border-radius:4px;font-size:0.85em;flex:1" value="' + esc(String(p.slot_duration || 60)) + '" data-change="onPlanningParam" data-args=\'' + _da("slot_duration") + '\' data-pass-value>';
    h += '<span style="font-size:0.78em;color:var(--text-muted)">min</span>';
    h += '</div></div>';
    h += '<div class="planning-param" style="display:flex;flex-direction:column;gap:4px">';
    h += '<label style="font-size:0.75em;font-weight:600;color:var(--text-muted);text-transform:uppercase">' + t("audit.planning.lunch_start") + '</label>';
    h += '<input type="time" style="padding:6px 8px;border:1px solid var(--border);border-radius:4px;font-size:0.85em" value="' + esc(p.lunch_start || "12:30") + '" data-change="onPlanningParam" data-args=\'' + _da("lunch_start") + '\' data-pass-value>';
    h += '</div>';
    h += '<div class="planning-param" style="display:flex;flex-direction:column;gap:4px">';
    h += '<label style="font-size:0.75em;font-weight:600;color:var(--text-muted);text-transform:uppercase">' + t("audit.planning.lunch_duration") + '</label>';
    h += '<div style="display:flex;align-items:center;gap:6px">';
    h += '<input type="number" min="0" max="120" step="15" style="padding:6px 8px;border:1px solid var(--border);border-radius:4px;font-size:0.85em;flex:1" value="' + esc(String(p.lunch_duration || 60)) + '" data-change="onPlanningParam" data-args=\'' + _da("lunch_duration") + '\' data-pass-value>';
    h += '<span style="font-size:0.78em;color:var(--text-muted)">min</span>';
    h += '</div></div>';
    h += '</div>';
    // ── Generate button ──
    ...
    if (D.planning.slots && D.planning.slots.length > 0) {
        ...
        D.planning.slots.forEach(function (slot, idx) {
            ...
            if (slot.date && slot.date !== currentDay) {
                ...
                h += '<div style="display:flex;align-items:center;gap:10px;margin-bottom:10px;padding-bottom:8px;border-bottom:2px solid var(--light-blue)">';
                h += '<span style="font-size:0.95em;font-weight:700;color:var(--blue)">' + esc(_formatDayHeader(slot.date, dayNum)) + '</span>';
                h += '</div>';
            }
            if (slot.type === "lunch") {
                // ── Lunch separator ──
                h += '<div style="display:flex;align-items:center;gap:10px;margin:8px 0;padding:8px 12px;background:#f5f5f5;border-radius:6px;border:1px dashed #ccc">';
                h += '<span class="slot-time" style="font-weight:600;font-family:monospace;font-size:0.82em;color:#95a5a6">' + esc(slot.start) + ' - ' + esc(slot.end) + '</span>';
                h += '<span style="font-size:0.82em;color:#95a5a6;font-style:italic">' + t("audit.planning.lunch") + '</span>';
                h += '</div>';
            }
            else {
                // ── Audit slot card ──
        ...
        // ── Export buttons ──
        h += '<div style="display:flex;gap:10px;justify-content:center;margin-top:20px;padding-top:16px;border-top:1px solid var(--border)">';
...
function exportPlanningCSV() {
    if (!D.planning.slots || D.planning.slots.length === 0)
        return;
    var rows = [["Date", "Debut", "Fin", "Type", "Domaine"]];
...
function exportPlanningWord() {
    ...
    html += '<h1 style="font-family:Calibri;color:#2c3e50">Planning d\'audit — ' + esc(D.meta.name || "") + '</h1>';
    html += '<p style="font-family:Calibri;color:#7f8c8d">Ref: ' + esc(D.meta.ref || "") + ' | Date: ' + esc(D.meta.date || "") + '</p>';
    html += '<table><tr><th>Horaire</th><th>Domaine</th></tr>';
    var curDay = "";
    D.planning.slots.forEach(function (s) {
        if (s.date !== curDay) {
            curDay = s.date;
        ...
        if (s.type === "lunch") {
            html += '<tr class="lunch"><td>' + esc(s.start) + ' - ' + esc(s.end) + '</td><td>Pause dejeuner</td></tr>';
        }
        else {
