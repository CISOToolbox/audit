...
function renderDocReview() {
    ...
    cats.forEach(function (cat) {
        ...
        docs.forEach(function (d) {
            ...
            if (d.critical)
                h += ' <span style="color:var(--red);font-size:0.7em;font-weight:700">★</span>';
            if (d.hds)
                h += ' <span style="font-size:0.6em;background:#3a8a6e;color:white;padding:1px 4px;border-radius:3px">HDS</span>';
            h += '</td>';
            ...
            if (d.linkedControls && d.linkedControls.length)
                h += '<div style="font-size:0.72em;color:var(--light-blue);margin-top:2px">§ ' + d.linkedControls.join(', ') + '</div>';
            if (showAlert)
                h += '<div style="font-size:0.75em;color:var(--red);margin-top:2px">⚠ ' + esc(d.ecartAuto) + '</div>';
            h += '</td>';
