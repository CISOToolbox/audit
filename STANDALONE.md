...
```
key = HKDF(JWT_SECRET, salt="ciso-suite/jwt-key/v1", info="ciso-module:audit")
```
...
`MODULE_NAME=audit` in `docker-compose.yml` is what selects that key and that
...
derivation exists to prevent. `MODULE_COOKIE=audit_token` likewise keeps the
...
`ENCRYPTION_KEY` seals the stored connector credentials (AES-GCM,
PBKDF2-SHA256). It is **required** — `docker compose` refuses to start without
it — and must be a **dedicated** value, distinct from `JWT_SECRET`. The former
...
./backup.sh                      # → backups/audit_<date>.sql.gz
./backup.sh --dir /srv/backups --keep 30

# Scheduled (cron) — daily at 02:00
0 2 * * *  cd /path/to/audit && ./backup.sh >> backups/backup.log 2>&1
...
./restore.sh backups/audit_2026-08-13_0200.sql.gz
```
...
- A dump taken on an **older** application version restores fine (Alembic
  ...
  `/api/version`).
- Volume snapshots of `audit-db-data` (VM/SAN level) also work as a
...
```ini
# /etc/systemd/system/audit-backup.service
[Service]
Type=oneshot
WorkingDirectory=/path/to/audit
ExecStart=/path/to/audit/backup.sh

# /etc/systemd/system/audit-backup.timer
[Timer]
OnCalendar=*-*-* 02:00:00
Persistent=true
[Install]
