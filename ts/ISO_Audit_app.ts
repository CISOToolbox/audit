...
function cardHTML(c: AuditControl, f: AuditFinding): string {
    ...
    h += '<span class="ctrl-title">' + esc(c.t) + '</span>';
    if (c.hds) h += '<span class="ctrl-hds">HDS</span>';
    h += '<div class="ctrl-actions">';
    if (QUESTIONS[c.id]) h += '<button class="btn-questions" data-click="toggleQuestions" data-args=\'' + _da(c.id) + '\'>' + t("audit.btn.questions") + '</button>';
    h += '</div>';
    h += '</div>';
    // Description
    h += '<div class="ctrl-desc">' + esc(c.desc) + '</div>';
    // Status buttons
    ...
    if (QUESTIONS[c.id]) {
        ...
        QUESTIONS[c.id].forEach(function(q) {
...
function renderDomain(domainId: string): void {
    ...
    controls.forEach(function(c) {
        ...
        if (_filterText) {
            ...
            if (c.id.toLowerCase().indexOf(q) < 0 && c.t.toLowerCase().indexOf(q) < 0 && c.desc.toLowerCase().indexOf(q) < 0 && (f.constats || "").toLowerCase().indexOf(q) < 0 && (f.preuve || "").toLowerCase().indexOf(q) < 0) return;
        }
        h += cardHTML(c, f);
        shown++;
    });
...
function renderDashboard(): void {
    ...
    // Charts row: Gauge + Donut + Radar
    h += '<div class="dash-charts-grid">';

    // Gauge
    h += '<div class="dash-card"><h3>' + t("audit.dash.gauge_title") + '</h3>';
    h += buildGauge(S);
    h += '<div style="font-size:0.72em;color:var(--text-muted);margin-top:6px">' + t("audit.dash.maturity_formula") + '</div>';
    h += '</div>';
    ...
    // Radar
    h += '<div class="dash-card"><h3>' + t("audit.dash.radar_title") + '</h3>';
    h += buildRadar(S);
    h += '</div>';

    h += '</div>'; // dash-charts-grid
    ...
    if (ecarts.length > 0) {
        ...
        h += '<table class="nc-table"><thead><tr><th>ID</th><th>' + t("audit.dash.nc_control") + '</th><th>Statut</th><th>' + t("audit.field.ecart_constat") + '</th><th>' + t("audit.field.ecart_action") + '</th></tr></thead><tbody>';
...
function buildDonut(S: AuditStats): string {
    ...
    var svg = '<svg viewBox="0 0 160 160" style="width:160px;height:160px;display:block;margin:0 auto">';
    var angle = -Math.PI / 2;
...
function buildRadar(S: AuditStats): string {
    ...
    var svg = '<svg viewBox="0 0 ' + size + ' ' + size + '" class="radar-svg" style="width:100%;max-width:420px;height:auto;display:block;margin:0 auto">';
...
function renderJournal(): void {
    ...
    if (!D.journal || D.journal.length === 0) {
        el.innerHTML = '<div style="color:var(--text-muted);text-align:center;padding:20px">' + t("audit.journal.empty") + '</div>';
        return;
    }
    var h = '';
    D.journal.forEach(function(entry) {
        ...
        if (entry.author) h += '<span style="font-size:0.8em;color:var(--text-muted)">' + esc(entry.author) + '</span>';
        h += '</div>';
    });
    el.innerHTML = h;
}
...
function onSearchInput(val: string): void {
    ...
    for (var i = 0; i < shown; i++) {
        ...
        if (r.f.status && STATUS_MAP[r.f.status]) {
            statusH = '<span class="search-result-status" style="background:' + statusColor(r.f.status) + '">' + esc(statusLabel(r.f.status)) + '</span>';
        }
        ...
        h += '<div class="search-result-title">' + esc(r.ctrl.t) + statusH + '</div>';
        h += '<div class="search-result-match"><em style="font-size:0.9em;color:var(--text-muted)">' + esc(r.matchField) + '</em> — ' + r.snippet + '</div>';
        h += '</div></div>';
    }
    if (results.length > 50) {
        h += '<div style="text-align:center;padding:10px;font-size:0.78em;color:var(--text-muted)">' + t("audit.search.more", { count: results.length - 50 }) + '</div>';
    }
...
function generateReport(): void {
    ...
    DOMAINS.forEach(function(dom) {
        var ds = S.domains[dom.id];
        domainLines += "- " + dom.label + " : " + ds.score + "% (" + ds.audited + "/" + ds.total + " audités)\n";
    });
    ...
    CONTROLS.forEach(function(c) {
        ...
        if (f.status === "ncmaj" || f.status === "ncmin") {
            ncLines += "- [" + f.status.toUpperCase() + "] " + c.id + " " + c.t + "\n";
    ...
    _aiCallAPI(systemPrompt, userPrompt).then(function(response) {
        ...
        p.footer.innerHTML = '<button class="pwd-btn pwd-ok" id="export-report-word" style="background:var(--light-blue)">' + t("audit.report.export_word") + '</button>'
