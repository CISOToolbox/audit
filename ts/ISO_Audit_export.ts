...
function exportCSV(): void {
    var CONTROLS: AuditControl[] = window.ISO_AUDIT_CONTROLS || [];
    var DOMAINS: AuditDomain[] = window.ISO_AUDIT_DOMAINS || [];
    if (!D || !D.meta) return;

    var domainMap: Record<string, string> = {};
    DOMAINS.forEach(function(d) { domainMap[d.id] = d.label; });
...
function _preloadAllImages(CONTROLS: AuditControl[], cb: (imageMap: Record<string, AuditImageEntry[]>) => void): void {
    var imageMap: Record<string, AuditImageEntry[]> = {}; // ctrlId -> [{id, data, name}]
    var ctrlsWithImages = CONTROLS.filter(function(c) {
        var f = readFinding(c.id);
        return f.images && f.images.length > 0;
    });
    if (ctrlsWithImages.length === 0) { cb(imageMap); return; }
    ...
    ctrlsWithImages.forEach(function(c) {
        _imgGetAll(c.id, function(imgs) {
            ...
            if (--pending === 0) cb(imageMap);
        });
    });
}

function _buildWordDoc(CONTROLS: AuditControl[], DOMAINS: AuditDomain[], imageMap: Record<string, AuditImageEntry[]>): void {
    var m = D.meta;
    ...
    CONTROLS.forEach(function(ctrl) {
        ...
        var s = f.status;
        if (s === "c") cCnt++;
        else if (s === "ncmaj") ncmajCnt++;
        else if (s === "ncmin") ncminCnt++;
        else if (s === "ps") psCnt++;
        else if (s === "pp") ppCnt++;
        else if (s === "na") naCnt++;
        else todoCnt++;
    ...
    function p(text: string, style?: string | null, opts?: AuditWordPOpts): string {
        opts = opts || {};
        var sz = opts.sz || "20";
    ...
    function pEmpty(after?: number): string { return '<w:p><w:pPr><w:spacing w:after="' + (after || 120) + '"/></w:pPr></w:p>'; }

    function heading(text: string, level: number): string {
        var configs: Record<number, AuditWordPOpts> = {
    ...
    function table(rows: AuditWordRow[], colWidths: number[]): string {
    ...
    function _sColor(s: string): string {
        return s === "c" ? "27AE60" : s === "ncmaj" ? "C0203A" : s === "ncmin" ? "C06000" : s === "ps" ? "6030C0" : s === "pp" ? "3498DB" : "888888";
    }

    function _sLabel(s: string): string {
        return statusLabel(s) || t("audit.status.na_export") || "N/A";
    }

    // ── Image tracking for OOXML ──
    var _imgRels: AuditWordImgRel[] = [];   // { rId, filename, base64 }
    var _imgIdx = 0;

    function addImage(dataUrl: string, widthCm?: number): string {
        if (!dataUrl) return "";
        _imgIdx++;
    ...
    DOMAINS.forEach(function(dom) {
        ...
        body += heading(dom.label, 1);
        body += hrLine();
        body += pEmpty();

        var domRows: AuditWordRow[] = [{ cells: [
        ...
        ctrls.forEach(function(ctrl) {
            if (!ctrl._ctrlImages || ctrl._ctrlImages.length === 0) return;
            body += heading(ctrl.id + " — Images preuves", 3);
            ctrl._ctrlImages.forEach(function(img) {
                if (img.data) {
                    if (img.name) body += p(img.name, null, { sz: "16", italic: true, color: "666666" });
    ...
    // ═══ NON-CONFORMITÉS ═══
    var ncSeverityOrder: Record<string, number> = { ncmaj: 0, ncmin: 1, ps: 2, pp: 3 };
    ...
    if (hasJpeg) ctExtras += '<Default Extension="jpeg" ContentType="image/jpeg"/>';
    if (hasPng) ctExtras += '<Default Extension="png" ContentType="image/png"/>';
...
function exportDocReviewCSV(): void {
    var DOC_REVIEW: AuditDocItem[] = window.ISO_AUDIT_DOC_REVIEW || [];
    if (!D || !D.doc_review) return;

    var statusMap: Record<string, string> = { recu: "Reçu", incomplet: "Incomplet", manquant: "Manquant", na: "N/A" };

    var rows = [["Ref", "Document", "Statut", "Observations"]];

    DOC_REVIEW.forEach(function(d) {
        var entry: Partial<AuditDocEntry> = D.doc_review[d.ref] || {};
        rows.push([
            d.ref,
            d.label,
