...
function _getDocEntry(ref: string): AuditDocEntry {
...
function renderDocReview(): void {
    ...
    DOC_REVIEW.forEach(function(d) {
        ...
        if (s) {
            ...
            if (s === "recu" || s === "incomplet" || s === "manquant" || s === "na") counts[s]++;
        }
    });
...
function cycleDocStatus(ref: string): void {
    _saveState();
...
function setDocObs(ref: string, val: string): void {
    var entry = _getDocEntry(ref);
