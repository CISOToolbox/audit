...
Un fichier de demonstration (`demo-fr.json`) est inclus avec l'application. Il contient un audit complet pour une societe fictive (MedSecure SAS) et permet d'explorer toutes les fonctionnalites.

### Utilisation

1. Ouvrir l'application dans un navigateur
2. Cliquer sur **Fichier > Ouvrir**
3. Selectionner le fichier `demo-fr.json`
4. Parcourir les domaines via la barre laterale

Sur la version en ligne, vous pouvez charger les donnees de demo depuis les Parametres (icone engrenage en haut a droite).
...
|--------|-------|-------------|
| `deploy-staging.sh` | staging.audit.cisotoolbox.org | Fichiers bruts (pas de minification) |
| `deploy.sh` | audit.cisotoolbox.org | JS/CSS/HTML minifies via terser |
