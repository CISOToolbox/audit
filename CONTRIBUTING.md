# Contributing to CISO Toolbox - Audit
...
Audit is developed in a private monorepo and published here. Three categories
of file coexist:
...
| Generated frontend audits | `app/js/*.js` and `app/css/*.css` carrying a `GENERATED` header | **No** |
...
header is module-specific build output, not a shared audit.
...
One of the end-to-end tests asserts that the shared frontend audits still carry
...
The module answers on <http://localhost:8086>.
