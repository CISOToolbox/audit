...
window.CT_CONFIG = {
    ...
    getSociete: function(d: AuditData) { return d && d.meta ? d.meta.name : ""; },
    getDate: function(d: AuditData) { return d && d.meta ? d.meta.date : ""; }
};
...
let D: AuditData = JSON.parse(JSON.stringify(window.ISO_AUDIT_INIT_DATA || {}));
const CONTROLS: AuditControl[] = window.ISO_AUDIT_CONTROLS || [];
const DOMAINS: AuditDomain[] = window.ISO_AUDIT_DOMAINS || [];
const QUESTIONS: Record<string, string[]> = window.ISO_AUDIT_QUESTIONS || {};
...
function statusColor(s: string): string { return STATUS_MAP[s] ? STATUS_MAP[s].color : "var(--ct-neutral)"; }
...
function ctrlT(c: AuditControl): string { return _rt(c as unknown as Record<string, any>, "t"); }
function ctrlDesc(c: AuditControl): string { return _rt(c as unknown as Record<string, any>, "desc"); }
function domLabel(d: AuditDomain): string { return _rt(d as unknown as Record<string, any>, "label"); }
function ctrlQuestions(id: string): string[] {
    var en = window.ISO_AUDIT_QUESTIONS_EN;
    if (_locale === "en" && en && en[id]) return en[id];
    return QUESTIONS[id] || [];
}
var _EMPTY_FINDING: AuditFinding = { status: "", preuve: "", constats: "", ecart_critere: "", ecart_constat: "", ecart_cause: "", ecart_action: "", images: [] };
function getFinding(id: string): AuditFinding {
    if (!D.findings[id]) D.findings[id] = { status: "", preuve: "", constats: "", ecart_critere: "", ecart_constat: "", ecart_cause: "", ecart_action: "", images: [] };
    if (!D.findings[id].images) D.findings[id].images = [];
    return D.findings[id];
}
// Read-only variant — does not mutate D.findings (for exports)
function readFinding(id: string): AuditFinding {
    return D.findings[id] || _EMPTY_FINDING;
}

function domainControls(domainId: string): AuditControl[] {
    return CONTROLS.filter(function(c) { return c.d === domainId; });
}

function isEcart(status: string): boolean { return status === "ncmaj" || status === "ncmin" || status === "ps" || status === "pp"; }

/** Garde de type : statut comptabilisé dans les stats (équivalent de l'ancien `S[s] !== undefined`). */
function _isStatusKey(s: string): s is AuditStatusKey {
...
function selectPanel(panelId: string): void {
    ...
    else if (panelId === "planning" && typeof renderPlanning === "function") renderPlanning();
    else if (panelId === "journal") renderJournal();
    else if (panelId === "measures" && typeof window._renderAuditMeasures === "function") window._renderAuditMeasures();
    else if (panelId === "history" && typeof renderHistory === "function") renderHistory();
}
...
function onMetaChange(field: string, val: string): void {
    _saveState();
    (D.meta as unknown as Record<string, string>)[field] = val;
    if (field === "name") document.getElementById("header-subtitle")!.textContent = val;
    _autoSave();
...
function renderMeta(): void {
    ...
    // Update meta inputs
    var fields: (keyof AuditMeta)[] = ["name", "ref", "date", "auditor", "scope"];
    fields.forEach(function(f) {
        var el = document.getElementById("meta-" + f) as HTMLInputElement | null;
        if (el && el.value !== (D.meta[f] || "")) el.value = D.meta[f] || "";
    });
    var hdsEl = document.getElementById("meta-hds") as HTMLSelectElement | null;
    if (hdsEl && hdsEl.value !== (D.meta.hds || "non")) hdsEl.value = D.meta.hds || "non";
}
...
function cardHTML(c: AuditControl, f: AuditFinding): string {
    ...
    if (c.hds) h += '<span class="ctrl-hds">HDS</span>';
    h += '<div class="ctrl-actions">';
    if (ctrlQuestions(c.id).length) h += '<button class="btn-questions" data-click="toggleQuestions" data-args=\'' + _da(c.id) + '\'>' + t("audit.btn.questions") + '</button>';
    h += '</div>';
    h += '</div>';
    // Description
    ...
    if (hasEcart) {
        ...
        if (typeof window._auditControlMeasuresHTML === "function") h += window._auditControlMeasuresHTML(c.id);
        h += '</div>';
    }
    // Questions panel
...
function renderDomain(domainId: string): void {
    ...
    controls.forEach(function(c) {
        ...
        // Apply filters
        if (_filterStatus === "_empty" && f.status !== "") return;
        if (_filterStatus && _filterStatus !== "_empty" && f.status !== _filterStatus) return;
        if (_filterHDS === "hds" && !c.hds) return;
        if (_filterText) {
            var q = _filterText.toLowerCase();
            if (c.id.toLowerCase().indexOf(q) < 0 && ctrlT(c).toLowerCase().indexOf(q) < 0 && ctrlDesc(c).toLowerCase().indexOf(q) < 0 && (f.constats || "").toLowerCase().indexOf(q) < 0 && (f.preuve || "").toLowerCase().indexOf(q) < 0) return;
        }
        h += cardHTML(c, f);
        shown++;
    });
...
function setStatus(ctrlId: string, status: string): void {
    _saveState();
    ...
    if (_currentPanel.startsWith("domain-")) renderDomain(_currentPanel.replace("domain-", ""));
    updateSidebarBadges();
    showStatus(t("audit.status.status_changed", { ctrl: ctrlId, status: statusLabel(f.status) || t("audit.status.non_audite") }));
}
window.setStatus = setStatus;

function setField(ctrlId: string, field: string, val: string): void {
    var f = getFinding(ctrlId);
    (f as unknown as Record<string, string>)[field] = val;
    _autoSave();
    logEntry("field", { ctrl: ctrlId, field: field });
}
window.setField = setField;

function onFilterStatus(val: string): void { _filterStatus = val; if (_currentPanel.startsWith("domain-")) renderDomain(_currentPanel.replace("domain-", "")); }
function onFilterHDS(val: string): void { _filterHDS = val; if (_currentPanel.startsWith("domain-")) renderDomain(_currentPanel.replace("domain-", "")); }
function onFilterText(val: string): void { _filterText = val; if (_currentPanel.startsWith("domain-")) renderDomain(_currentPanel.replace("domain-", "")); }
window.onFilterStatus = onFilterStatus;
window.onFilterHDS = onFilterHDS;
window.onFilterText = onFilterText;

function toggleQuestions(ctrlId: string): void {
    var panel = document.getElementById("questions-" + ctrlId);
    if (panel) panel.classList.toggle("open");
}
window.toggleQuestions = toggleQuestions;

function copyQuestion(text: string): void {
...
function computeStats(): AuditStats {
    var S: AuditStats = { total: CONTROLS.length, audited: 0, c: 0, ncmaj: 0, ncmin: 0, ps: 0, pp: 0, na: 0, score: 0, grade: "", gradeColor: "", domains: {} };
    CONTROLS.forEach(function(ctrl) {
        var f = getFinding(ctrl.id);
        if (f.status) {
            S.audited++;
            if (_isStatusKey(f.status)) S[f.status]++;
        }
    });
    ...
    DOMAINS.forEach(function(dom) {
        ...
        var ds: AuditDomainStats = { total: ctrls.length, audited: 0, c: 0, ncmaj: 0, ncmin: 0, ps: 0, pp: 0, na: 0, score: 0 };
        ctrls.forEach(function(ctrl) {
            var f = getFinding(ctrl.id);
            if (f.status) {
                ds.audited++;
                if (_isStatusKey(f.status)) ds[f.status]++;
            }
        });
...
function renderDashboard(): void {
    ...
    // NC table
    var ecarts: { ctrl: AuditControl; finding: AuditFinding }[] = [];
    CONTROLS.forEach(function(c) {
        var f = getFinding(c.id);
        if (isEcart(f.status)) ecarts.push({ ctrl: c, finding: f });
    });
    // Sort by severity: ncmaj first, then ncmin, ps, pp
    var sevOrder: Record<string, number> = { ncmaj: 0, ncmin: 1, ps: 2, pp: 3 };
...
function buildGauge(S: AuditStats): string {
    var score = S.score;
...
function buildDonut(S: AuditStats): string {
    var data: { label: string; value: number; color: string }[] = [
    ...
    if (todo > 0) data.push({ label: t("audit.dash.non_audited"), value: todo, color: "var(--ct-line)" });
    if (data.length === 0) return '<div style="text-align:center;color:var(--ct-ink-2);padding:20px">&mdash;</div>';
...
function buildRadar(S: AuditStats): string {
    var size = 500;
    ...
    function polarToXY(idx: number, radius: number): { x: number; y: number } {
...
function buildStackedBars(S: AuditStats): string {
    var h = '';
    DOMAINS.forEach(function(dom) {
        var ds = S.domains[dom.id];
        if (!ds) return;
        ...
        segs.forEach(function(seg) {
            if (seg.val > 0) h += '<div class="stacked-seg" style="width:' + (seg.val / total * 100).toFixed(1) + '%;background:' + seg.color + '"></div>';
        });
        h += '</div></div>';
    });
    return h;
...
function buildHDSBreakdown(S: AuditStats): string {
    var hds: AuditHDSCount = { total: 0, c: 0, nc: 0, other: 0 };
    var nonhds: AuditHDSCount = { total: 0, c: 0, nc: 0, other: 0 };
    CONTROLS.forEach(function(ctrl) {
        var f = getFinding(ctrl.id);
        var target = ctrl.hds ? hds : nonhds;
        target.total++;
        if (f.status === "c") target.c++;
        else if (f.status === "ncmaj" || f.status === "ncmin") target.nc++;
        else if (f.status) target.other++;
    });
...
function buildHDSBar(F: AuditHDSCount, label: string): string {
    var total = F.total || 1;
    var rows = [
...
function buildDomainBars(S: AuditStats): string {
    ...
    DOMAINS.forEach(function(dom) {
        ...
        if (ds.c > 0) h += '<div style="position:absolute;height:100%;width:' + (ds.c / ds.total * 100) + '%;background:var(--ct-low)"></div>';
        if (ds.ncmaj > 0) h += '<div style="position:absolute;height:100%;left:' + (ds.c / ds.total * 100) + '%;width:' + (ds.ncmaj / ds.total * 100) + '%;background:var(--ct-critical)"></div>';
        if (ds.ncmin > 0) h += '<div style="position:absolute;height:100%;left:' + ((ds.c + ds.ncmaj) / ds.total * 100) + '%;width:' + (ds.ncmin / ds.total * 100) + '%;background:var(--ct-high)"></div>';
        if (ds.ps > 0) h += '<div style="position:absolute;height:100%;left:' + ((ds.c + ds.ncmaj + ds.ncmin) / ds.total * 100) + '%;width:' + (ds.ps / ds.total * 100) + '%;background:var(--ct-medium)"></div>';
        if (ds.pp > 0) h += '<div style="position:absolute;height:100%;left:' + ((ds.c + ds.ncmaj + ds.ncmin + ds.ps) / ds.total * 100) + '%;width:' + (ds.pp / ds.total * 100) + '%;background:var(--ct-info)"></div>';
        h += '</div>';
...
function logEntry(type: string, data?: { ctrl?: string; status?: string; field?: string }): void {
    ...
    if (D.journal.length > 2000) D.journal.length = 2000;
}
...
function renderJournal(): void {
    ...
    D.journal.forEach(function(entry) {
        ...
        if (entry.data) {
            ...
            if (entry.data.status) text += " → " + statusLabel(entry.data.status);
            if (entry.data.field) text += "." + entry.data.field;
        }
...
function onSearchScope(val: string): void {
    _searchScope = val;
    onSearchInput((document.getElementById("search-input") as HTMLInputElement).value || "");
}
...
function onSearchInput(val: string): void {
    ...
    var results: { ctrl: AuditControl; f: Partial<AuditFinding>; matchField: string; snippet: string }[] = [];
    var reEscape = q.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    var re = new RegExp('(' + reEscape + ')', 'gi');

    CONTROLS.forEach(function(ctrl) {
        var f: Partial<AuditFinding> = D.findings[ctrl.id] || {};
        // Filter by scope
        if (_searchScope === "nc" && f.status !== "ncmaj" && f.status !== "ncmin") return;

        var fields: { k: string; v: string }[] = [];
        if (_searchScope === "all") {
        ...
        var matchField: string | null = null, matchVal = "";
...
function goToSearchResult(ctrlId: string): void {
    closeSearch();
...
document.addEventListener("keydown", function(e) {
    ...
    if (e.key === "Enter" && document.activeElement && document.activeElement.id === "search-input") {
        var first = document.querySelector(".search-result-item") as HTMLElement | null;
...
function generateReport(): void {
    ...
    CONTROLS.forEach(function(c) {
        ...
        if (f.status === "ncmaj" || f.status === "ncmin") {
            ...
            if (f.ecart_constat) ncLines += "  Constat: " + f.ecart_constat + "\n";
            if (f.ecart_cause) ncLines += "  Cause: " + f.ecart_cause + "\n";
            if (f.ecart_action) ncLines += "  Action: " + f.ecart_action + "\n";
        }
    });
...
function _exportAIReportAsWord(text: string): void {
    ...
    _loadAsset("js/vendor/jszip.min.js", function() {
        ...
        function wp(txt: string, opts?: AuditWordPOpts): string {
            opts = opts || {};
            var sz = opts.sz || "22";
...
function ensureKeys(): void {
    if (!D.meta) D.meta = {} as AuditMeta;
    var metaDefaults: Record<string, string> = { name: "", ref: "", date: "", auditor: "", scope: "", hds: "non" };
    var meta = D.meta as unknown as Record<string, string>;
    for (var k in metaDefaults) { if (!(k in meta)) meta[k] = metaDefaults[k]; }
    if (!D.findings) D.findings = {};
    if (!D.doc_review) D.doc_review = {};
    if (!D.journal) D.journal = [];
    if (!D.timers) D.timers = {};
    if (!D.planning) D.planning = { params: {}, slots: [] };
}
...
async function renderHistory(): Promise<void> {
    // Frontend : cisotoolbox_local fournit toujours le panneau de snapshots.
    await _renderSnapshotsPanel!({
        target: "history-content",
        orgField: "societe",
        keys: {
            create: "audit.history.create",
            encrypt: "audit.history.encrypt",
            decrypt: "audit.history.decrypt",
            encryption_active: "audit.history.encryption_active",
            none: "audit.history.none",
            col_name: "audit.history.col_name",
            col_date: "audit.history.col_date",
            col_org: "audit.history.col_client",
            col_actions: "audit.history.col_actions",
            restore: "audit.history.restore",
            export: "audit.history.export",
            hint: "audit.history.hint"
        }
    });
}
...
function renderAll(): void {
    try {
        ...
        if (menuReport) menuReport.style.display = (typeof _aiIsEnabled === "function" && _aiIsEnabled()) ? "" : "none";
        renderMeta();
        updateSidebarBadges();
        // Re-render current panel
        if (_currentPanel === "dashboard") renderDashboard();
        else if (_currentPanel.startsWith("domain-")) renderDomain(_currentPanel.replace("domain-", ""));
        else if (_currentPanel === "journal") renderJournal();
        else if (_currentPanel === "docreview" && typeof renderDocReview === "function") renderDocReview();
        else if (_currentPanel === "planning" && typeof renderPlanning === "function") renderPlanning();
        else if (_currentPanel === "history" && typeof renderHistory === "function") renderHistory();
    } catch (e) {
        console.error("renderAll error:", e);
    }
}

function _initDataAndRender(afterFn?: () => void): void {
