# CISO Toolbox - Audit (standalone)

ISO/IEC 27001:2022 internal audit: clauses §4–§10 and Annex A assessment, document review, audit schedule, corrective actions, stored audits with import of files from the browser-local version.

This repository is the **standalone** packaging of the Audit module: one
...
versions* below), hosted at <https://audit.cisotoolbox.org> — data never leaves your browser.
...
- **ISO 27001 audit workflow**: stored audits with scope, criteria and
  schedule; findings with severity and evidence; audit reports.
- **Corrective actions**: tracked with owner, control reference, due date,
  status and journal.
- **Frontend-file import**: open a JSON file produced by the browser-only
  app and continue server-side with accounts and persistence.
- **Interoperable data**: JSON import/export; MedSecure demo dataset
  included.
...
The module is then served on <http://localhost:8086>.
...
```
key = HKDF-SHA256(JWT_SECRET, salt="ciso-suite/jwt-key/v1", info="ciso-module:audit")
```

and tokens carry `iss=ciso-audit` / `aud=ciso-module:audit`. A session minted by
...
`MODULE_NAME=audit` and `MODULE_COOKIE=audit_token` in `docker-compose.yml` are
...
| `ENCRYPTION_KEY` | yes | Dedicated AES key for the credentials stored at rest. **No fallback on `JWT_SECRET`** - compose refuses to start without it. |
